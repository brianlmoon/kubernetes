...
class JobSpec extends \Moonspot\Kubernetes\BaseObject {

    /**
     * Specifies the duration in seconds relative to the startTime that the job
     * may be continuously active before the system tries to terminate it;
     * value must be positive integer. If a Job is suspended (at creation or
     * through an update), this timer will effectively be stopped and reset
     ...
     * been .spec.completions successfully completed Pods. Each Pod completion
     * is homologous to each other.
     * 
     * `Indexed` means that the Pods of a Job get an associated completion
     * index from 0 to (.spec.completions - 1), available in the annotation
     ...
     * should be run with.  Setting to null means that the success of any pod
     ...
     * execution is terminated. When left as null the job continues execution
    ...
    /**
     * Specifies the policy of handling failed pods. In particular, it allows
     * to specify the set of actions and conditions which need to be satisfied
     * to take the associated action. If empty, the default behaviour applies -
     * the counter of failed pods, represented by the jobs's .status.failed
     * field, is incremented and it is checked against the backoffLimit. This
     * field cannot be used in combination with restartPolicy=OnFailure.
     * 
     ...
     * Job controller. If a Job is suspended after creation (i.e. the flag goes
