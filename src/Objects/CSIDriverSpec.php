...
use Moonspot\Kubernetes\Objects\Sets\TokenRequestSet;
use Moonspot\Kubernetes\Objects\Sets\StringSet;

class CSIDriverSpec extends \Moonspot\Kubernetes\BaseObject {
     ...
     * is only required for drivers which support both the "Persistent" and
     * "Ephemeral" VolumeLifecycleMode. Other drivers can leave pod info
     * disabled and/or ignore this field. As Kubernetes 1.15 doesn't support
     * this field, drivers can only support one mode when deployed on such a
     * cluster and the deployment determines which mode that is, for example
     ...
     * this CSI driver can be mounted separately with different `-o context`
     * options. This is typical for storage backends that provide volumes as
     * filesystems on block devices or as independent shared volumes.
     ...
     * 
     * When "false", Kubernetes won't pass any special SELinux mount options to
     * the driver. This is typical for volumes that represent subdirectories of
     * a bigger shared filesystem.
     * 
     * Default is "false".
     */
     ...
     * driver deployment has published some suitable CSIStorageCapacity object.
     * 
     * Alternatively, the driver can be deployed with the field unset or false
     * and it can be flipped later when storage capacity information has been
     * published.
     * 
    ...
    public function __construct() {
        $this->tokenRequests = new TokenRequestSet();
        $this->volumeLifecycleModes = new StringSet();
    }
}
