...
class LimitedPriorityLevelConfiguration extends \Moonspot\Kubernetes\BaseObject {

    /**
     * `borrowingLimitPercent`, if present, configures a limit on how many
     * seats this priority level can borrow from other priority levels. The
     * limit is known as this level's BorrowingConcurrencyLimit (BorrowingCL)
     * and is a limit on the total number of seats that this level may borrow
     * at any one time. This field holds the ratio of that limit to the level's
     * nominal concurrency limit. When this field is non-nil, it must hold a
     ...
     * priority level can borrow a number of seats that is greater than its own
     * nominal concurrency limit (NominalCL). When this field is left `nil`,
     ...
     * can be borrowed by other priority levels. The value of this field must
     * be between 0 and 100, inclusive, and it defaults to 0. The number of
     * seats that other levels can borrow from this level, known as this
     * level's LendableConcurrencyLimit (LendableCL), is defined as follows.
     * 
    ...
    public function __construct() {
        $this->limitResponse = new LimitResponse();
    }
}
