...
class MutatingWebhook extends \Moonspot\Kubernetes\BaseObject {
     ...
     * even via another API group or version. For example, if deployments can
     * be modified via apps/v1, apps/v1beta1, and extensions/v1beta1, and
     * "rules" only included `apiGroups:["apps"], apiVersions:["v1"],
     ...
     * 
     * IfNeeded: the webhook will be called at least one additional time as
     * part of the admission evaluation if the object being admitted is
     ...
     * invocations is not guaranteed to be exactly one. * if additional
     * invocations result in further modifications to the object, webhooks are
     * not guaranteed to be invoked again. * webhooks that use this option may
     * be reordered to minimize the number of additional invocations. * to
     * validate an object after all mutations are guaranteed complete, use a
     ...
     */
    public ?RuleWithOperationsSet $rules = null;
