...
use Moonspot\Kubernetes\Objects\Sets\NetworkPolicyEgressRuleSet;
use Moonspot\Kubernetes\Objects\Sets\NetworkPolicyIngressRuleSet;
use Moonspot\Kubernetes\Objects\Sets\StringSet;

class NetworkPolicySpec extends \Moonspot\Kubernetes\BaseObject {
     ...
     * ensure that the pods it selects are isolated by default). This field is
     * beta-level in 1.8
     */
    public ?NetworkPolicyEgressRuleSet $egress = null;

    /**
     * ingress is a list of ingress rules to be applied to the selected pods.
     * Traffic is allowed to a pod if there are no NetworkPolicies selecting
     * the pod (and cluster policy otherwise allows the traffic), OR if the
     * traffic source is the pod's local node, OR if the traffic matches at
     ...
     */
    public ?NetworkPolicyIngressRuleSet $ingress = null;

    /**
     * podSelector selects the pods to which this NetworkPolicy object applies.
     * The array of ingress rules is applied to any pods selected by this
     * field. Multiple network policies can select the same set of pods. In
     * this case, the ingress rules for each are combined additively. This
     * field is NOT optional and follows standard label selector semantics. An
     ...
     * "Egress" ]. Likewise, if you want to write a policy that specifies that
     * no egress is allowed, you must specify a policyTypes value that include
     * "Egress" (since such a policy would not include an egress section and
     * would otherwise default to just [ "Ingress" ]). This field is beta-level
     * in 1.8
     */
    public ?StringSet $policyTypes = null;

    public function __construct() {
        $this->egress = new NetworkPolicyEgressRuleSet();
        $this->ingress = new NetworkPolicyIngressRuleSet();
        $this->podSelector = new LabelSelector();
        $this->policyTypes = new StringSet();
    }
}
