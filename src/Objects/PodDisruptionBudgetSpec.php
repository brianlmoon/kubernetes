...
class PodDisruptionBudgetSpec extends \Moonspot\Kubernetes\BaseObject {
     ...
     * but not yet healthy can be evicted only if the guarded application is
     * not disrupted (status.currentHealthy is at least equal to
     * status.desiredHealthy). Healthy pods will be subject to the PDB for
     * eviction.
     * 
     * AlwaysAllow policy means that all running pods (status.phase="Running"),
     * but not yet healthy are considered disrupted and can be evicted
     * regardless of whether the criteria in a PDB is met. This means
     * perspective running pods of a disrupted application might not get a
