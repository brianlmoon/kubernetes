...
class ValidatingAdmissionPolicyBindingSpec extends \Moonspot\Kubernetes\BaseObject {
     ...
     * FailurePolicy of the ValidatingAdmissionPolicy applied. If the policy
     * does not specify a ParamKind then this field is ignored, and the rules
     ...
     * Fail, otherwise the failures are ignored. This includes compilation
     ...
     * client in HTTP Warning headers, with a warning code of 299. Warnings can
     * be sent both for allowed or denied admission responses.
     * 
     * "Audit" specifies that a validation failure is included in the published
     * audit event for the request. The audit event will contain a
     * `validation.policy.admission.k8s.io/validation_failure` audit annotation
     * with a value containing the details of the validation failures,
     * formatted as a JSON list of objects, each with the following fields: -
     * message: The validation failure message string - policy: The resource
     * name of the ValidatingAdmissionPolicy - binding: The resource name of
     * the ValidatingAdmissionPolicyBinding - expressionIndex: The index of the
     ...
     * 
     * Clients should expect to handle additional values by ignoring any values
     * not recognized.
     * 
     * "Deny" and "Warn" may not be used together since this combination
     * needlessly duplicates the validation failure both in the API response
     * body and the HTTP warning headers.
     * 
     * Required.
     */
    public ?StringSet $validationActions = null;

    public function __construct() {
        $this->matchResources = new MatchResources();
        $this->paramRef = new ParamRef();
        $this->validationActions = new StringSet();
    }
}
