...
class ValidationRule extends \Moonspot\Kubernetes\BaseObject {
     ...
     * string. If both message and messageExpression are present on a rule,
     ...
     * spaces/string with line breaks will be logged. messageExpression has
     * access to all the same variables as the rule; the only difference is the
     * return type. Example: "x must be less than max ("+string(self.max)+")"
     */
    public ?string $messageExpression = null;

    /**
     * optionalOldSelf is used to opt a transition rule into evaluation even
     * when the object is first created, or if the old object is missing the
     * value.
     * 
     * When enabled `oldSelf` will be a CEL optional whose value will be `None`
     ...
     * the x-kubernetes-validations extension in the schema. The `self`
     * variable in the CEL expression is bound to the scoped value. Example: -
     ...
     * accessible via `self[mapKey]`, map containment can be checked via
     * `mapKey in self` and all entries of the map are accessible via CEL
     * macros and functions such as `self.all(...)`. If the Rule is scoped to
     * an array, the elements of the array are accessible via `self[i]` and
     * also by macros and functions. If the Rule is scoped to a scalar, `self`
     * is bound to the scalar value. Examples: - Rule scoped to a map of
     * objects: {"rule": "self.components['Widget'].priority < 10"} - Rule
     * scoped to a list of integers: {"rule": "self.values.all(value, value >=
     * 0 && value < 100)"} - Rule scoped to a string value: {"rule":
     * "self.startsWith('kube')"}
     * 
     * The `apiVersion`, `kind`, `metadata.name` and `metadata.generateName`
     * are always accessible from the root of the object and from any
     ...
     * expressions. This includes: - Unknown field values that are preserved by
     * object schemas with x-kubernetes-preserve-unknown-fields. - Object
     * properties where the property schema is of an "unknown type". An
     * "unknown type" is recursively defined as:
     *   - A schema with no type and x-kubernetes-preserve-unknown-fields set
     * to true
     *   - An array where the items schema is of an "unknown type"
     *   - An object where the additionalProperties schema is of an "unknown
     * type"
     * 
