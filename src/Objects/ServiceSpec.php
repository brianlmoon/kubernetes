...
use Moonspot\Kubernetes\Objects\Sets\ServicePortSet;

class ServiceSpec extends \Moonspot\Kubernetes\BaseObject {
     ...
     * https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
     */
    public ?StringSet $clusterIPs = null;
     ...
     */
    public ?StringSet $externalIPs = null;
     ...
     * all endpoints evenly (possibly modified by topology and other features).
     * Note that traffic sent to an External IP or LoadBalancer IP from within
     * the cluster will always get "Cluster" semantics, but clients sending to
     * a NodePort from within the cluster may need to take traffic policy into
     ...
     * they receive on the ClusterIP. If set to "Local", the proxy will assume
     * that pods only want to talk to endpoints of the service on the same node
     * as the pod, dropping the traffic if there are no local endpoints. The
     ...
     * the primary IP family of the Service. Valid values are "IPv4" and
     * "IPv6".  This field only applies to Services of types ClusterIP,
     * NodePort, and LoadBalancer, and does apply to "headless" services. This
     ...
     */
    public ?StringSet $ipFamilies = null;
     ...
     * IP families on dual-stack configured clusters, otherwise fail). The
     ...
     * This field can only be set when the Service type is 'LoadBalancer'. If
     * not set, the default load balancer implementation is used, today this is
     * typically done through the cloud provider integration, but should apply
     * for any default implementation. If set, it is assumed that a load
     ...
     * creating or updating a Service to type 'LoadBalancer'. Once set, it can
     * not be changed. This field will be wiped when a service is updated to a
     ...
     * https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
     */
    public ?ServicePortSet $ports = null;
    ...
    public function __construct() {
        $this->clusterIPs = new StringSet();
        $this->externalIPs = new StringSet();
        $this->ipFamilies = new StringSet();
